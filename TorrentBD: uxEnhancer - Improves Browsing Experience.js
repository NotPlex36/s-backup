...
(function () {
  ...
  const Utils = {
    ...
    showToast: (message) => {
      ...
      Object.assign(toast.style, {
        position: 'fixed', top: '5%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: '#fff', color: '#333', padding: '15px', borderRadius: '500px',
        boxShadow: '0 2px 10px rgba(0, 0, 0, 0.3)', opacity: '0', transition: 'opacity 0.15s ease-in-out', zIndex: '1000', textAlign: 'center', fontSize: '14px'
      });
    ...
    const showEfficiency = () => {
      ...
      if (sizeNode && hourlySeedbonusNode) {
        ...
        if (![...document.querySelectorAll('.cr-wrapper .cr-label')].some(el => el.textContent.includes('Efficiency'))) {
          ...
          newWrapper.innerHTML = `<div class="cr-label">Efficiency</div><div class="cr-value"> : ${efficiency} SB/GiB</div>`;
    ...
    const showFlexZone = () => {
      ...
      const createFlexZoneRow = (label, value) => `
        <div class="cr-wrapper">
          ...
          <div class="cr-value" >: ${label === 'BonusRate' ? `<a title="Go To SeedBonus Breakdown" href="/seedbonus-breakdown.php">${value}</a>` : value}</div>
        </div>
      `;
      ...
      if (linksSection) {
        ...
        flexZoneSection.innerHTML = `
          ...
          <div class="margin-b-10">
            ...
            ${createFlexZoneRow('Popularity', `${+popularity*10}%`)}
  ...
  const seedbonusPageTweaks = () => {
    const showSeedbonusEstimation = () => {
      ...
      const displayPoints = match
        ? `${+parseFloat(match[1]).toFixed(2)} <span class="red100">x${parseInt(match[3], 10)}=${+parseFloat(match[4]).toFixed(2)}</span>`
  ...
  const shoutboxTweaks = () => {
    ...
    const handleActiveFocus = (shoutboxContainer, shoutInput) => {
      ...
      ['mouseover'].forEach(event =>
        shoutboxContainer.addEventListener(event, () => !window.getSelection().toString() && focusInput())
      );
    }
  ...
  const showTradeSummary = () => {
    ...
    const createTable = (categorySums) => {
      ...
      const table = createElement('table', null, { className: 'bordered simple-data-table', marginLeft: 'auto', marginRight: 'auto', width: '700px' });
